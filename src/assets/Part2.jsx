...
function Part2({ switchToLogin }) {
    ...
    return (
        <div className={`flex justify-center items-center min-h-screen p-4 ${darkMode ? 'bg-gray-900' : 'bg-white'}`}>
            <div className={`w-full md:w-[90%] lg:w-[60vw] h-auto md:min-h-[70vh] flex flex-col md:flex-row rounded-[2rem] md:rounded-l-[5rem] overflow-hidden shadow-[0_0_35px_rgba(0.5,0.5,0.5,0.3)] md:shadow-[0_0_55px_rgba(0.5,0.5,0.5,0.5)] ${darkMode ? 'shadow-gray-800' : ''}`}>
                {/* Left side */}
                <div className={`flex-1 ${darkMode ? 'bg-[#cc3345]' : 'bg-[#FF4457]'} flex flex-col justify-center items-center text-white p-6 md:p-8`}>
                    ...
                    <h4 className="text-lg md:text-2xl mb-4 md:mb-2 text-center">Already have an account? Login here!</h4>
                    <button
                        className={`bg-white text-[#FF4457] px-5 py-1.5 md:px-6 md:py-2 rounded-full font-semibold transition duration-300 cursor-pointer text-sm md:text-base ${darkMode ? 'hover:bg-gray-300' : 'hover:bg-gray-200'}`}
                ...
                <div className={`flex-1 ${darkMode ? 'bg-gray-800 text-white' : 'bg-white'} flex flex-col justify-center items-center py-8 md:py-0`}>
                    <h1 className="text-3xl md:text-4xl font-bold mb-6 md:m-[1em] md:-mt-[1em]">SIGN UP</h1>
